...
enum class FUNC_CHOICE { AIRDENSITYTEMP,SVP,SVP_DAVID,DSVP,CONC_VAPOUR,CASUN_INIT,SUNDAY_INIT, THETA_ANGLE,
	POTRISDAILY, POTRISHOUR, DAYLENGTH, SLOPE_CORR, ATM_CORR, TEMP_VPS,WETBULB, PM, PM_RAD, PM_VENT ,
...
STABILITY_HEAT, STABILITY_MOM, STABILITY_ANALYTICAL,
MONIN_OBUDKOV_PAR_RICHARD, FRICTION_VELOCITY, RES_AIR_HEAT_GENERAL, WINDPROFILE, RADLONG_OUT, AIRTEMP_DAY, AIRTEMP_YEAR,
SOILTEMP_ANALYTICAL_DAY, SOILTEMP_ANALYTICAL_YEAR
};

class FunctorClass 
{
public:
	FunctorClass(enum class FUNC_CHOICE x) : OPTION(x) {}
	...
	double operator() (double DiffZ, double TempAir, double DiffTemp, double WindSpeed, double zom, double zoh, double etaold)
	{
 	...
 	bool operator() (enum class FUNC_CHOICE choice, double a, double b, double c, double d) {
		//CASuNInit
		OPTION = choice;
	...
	bool operator() (enum class FUNC_CHOICE choice, double x) {
	...
	double operator() (enum class FUNC_CHOICE n) {
		OPTION = n;
	};
private:
