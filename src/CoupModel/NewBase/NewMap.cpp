...
bool NewMap::WriteOutputSummaryToXmlFile(pugi::xml_node node)
{
	...
	for (size_t i = 1; i < 5; i++) {
		pvv.clear();
		vp = GetPtrVector(enum simtype(i), false);
		out = SelectedOutVector(vp);
		for (size_t i = 0; i < out.size(); i++)
			pvv.push_back(out[i]);

		vp = GetPtrVector(enum simtype(i+7), false);
		out = SelectedOutVector(vp);
		for (size_t i = 0; i < out.size(); i++)
			pvv.push_back(out[i]);
		if (pvv.size() > 0) {
...
bool NewMap::Read_OutputSummary_FromXmlFile(pugi::xml_node node)
{
	...
	for (pugi::xml_node level1 = node.first_child(); level1; level1 = level1.next_sibling()) {
		...
		if (level1name == "State") type_id = enum simtype(1);
		else if (level1name == "Flow") type_id = enum simtype(2);
		else if (level1name == "Auxiliary") type_id = enum simtype(3);
		else if (level1name == "Driving") type_id = enum simtype(4);
		...
		for (pugi::xml_node level2 = level1.first_child(); level2; level2 = level2.next_sibling()) {
			...
			if (varstr.rfind("_") != string::npos) {
				...
				if (pVector != nullptr)  pVector->AddOutPutVar(output.LocalIndex-1, output);
				pVector->SetStoreFlag(output.LocalIndex - 1, output.StoreFlag);
			}
			else {
				pSimB = GetPtr(enum simtype(type_id+7),varstr);
				if (pSimB == nullptr) {
					string newname = p_ModelInfo->getOrigName(varstr);
					pSimB = GetPtr(enum simtype(type_id+7), newname);
					if (pSimB == nullptr) {
		...
		if (typ=="Parameters") {
			...
			for (auto it = vp.begin(); it != vp.end(); it++) {
				...
				if (original) {
					...
					for (size_t i = 0; i < pP->GetSize(); ++i) {
						if (pP->IsNotOriginalValue(i)) default_value = false;
					}

					if(!default_value) m_Pt_Array.push_back(pP);
				}
				else if (GroupSet == "ALL") {
...
bool NewMap::MakeMapComplete() {
	...
	vptr = GetPtrVector(SWITCH, true);
	p_ModelInfo->DefineSwitchMap(vptr);

	vptr = GetPtrVector(PAR_SINGLE, true);
	p_ModelInfo->DefineSingleParMap(vptr);

	vptr = GetPtrVector(PAR_TABLE, true);
	p_ModelInfo->DefineVectorParMap(vptr);

	vector<SimB*> vall;
	vall = GetPtrVector(STATE, true);

	vptr = GetPtrVector(FLOW, true);
	for (SimB* v : vptr) vall.push_back(v);

	vptr = GetPtrVector(AUX, true);
	for (SimB* v : vptr) vall.push_back(v);

	vptr = GetPtrVector(DRIVE, true);
	for (SimB* v : vptr) vall.push_back(v);

	p_ModelInfo->DefineVectorOutputMap(vall);

	vall = GetPtrVector(STATE_SINGLE, true);

	vptr = GetPtrVector(FLOW_SINGLE, true);
	for (SimB* v : vptr) vall.push_back(v);

	vptr = GetPtrVector(AUX_SINGLE, true);
	for (SimB* v : vptr) vall.push_back(v);

	vptr = GetPtrVector(DRIVE_SINGLE, true);
	for (SimB* v : vptr) vall.push_back(v);

	p_ModelInfo->DefineSingleOutputMap(vall);

	vptr = GetPtrVector(PGFILE, true);
	
	p_ModelInfo->DefineTimeSeriesMap(vptr);
